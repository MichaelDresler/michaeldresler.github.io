...
const RandomImages = () => {
  ...
  const changeOnClick = (index: number) => {
    ...
    animate(isLargeScreen ? yValue : xValue, -index * 60, {
      type: "spring",
      stiffness: 300,
      damping: 30,
    });
  ...
  const handleWheel = (event: React.WheelEvent) => {
    ...
    setScrollAccumulator((prevAccumulator) => {
      ...
      if (Math.abs(newAccumulator) >= 225) {
        // Adjust this value as needed
        const direction = newAccumulator > 0 ? 1 : -1;
        const newIndex = Math.max(
          0,
          Math.min(images.length - 1, imageIndex + direction),
        );
  ...
  return (
    <div
      onWheel={handleWheel}
      className="relative w-screen overflow-y-hidden h-[calc(100dvh-var(--padding-top))] overflow-hidden box-content"
    >
