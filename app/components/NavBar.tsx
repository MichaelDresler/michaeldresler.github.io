    ...
    <div className="fixed w-full left-0 top-0 text-sm px-6 sm:px-12  z-[1000]">
      ...
      >
        ...
        <div className="flex sm:ml-auto justify-center ">
          <ul className="flex flex-col justify-center sm:flex-row sm:gap-4 md:gap-8 font-GeistMono mt-8 sm:mt-0 items-center uppercase">
            {navlinks.map((link , index)=>(
              <li className="relative flex gap-12" key={index}>
                <Link className={` text-sm ${pathname === link.url? "text-text-brand-primary": "text-white/60"}`} href={link.url}>{link.name}</Link>
                <span className={` text-sm absolute pointer-events-none inset-[-8px] ${pathname === link.url && "  -z-10"}`}>
                  <div className={` ${pathname === link.url ? "flex": "hidden"}`}>
